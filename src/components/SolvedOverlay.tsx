...
import { Trophy, Timer, User, ArrowRight, RotateCcw, Loader2, Share2 } from 'lucide-react';
...
const SolvedOverlay: React.FC<Props> = ({
  ...
  const handleShare = async () => {
    const text = `I solved a ${GRID[difficulty].desc} Snapzzle in ${fmt(time)}! 🧩`;
    const url = 'https://snapzzleplay.web.app';
    if (navigator.share) {
      try { await navigator.share({ title: 'Snapzzle', text, url }); } catch { /* user cancelled */ }
    } else {
      try {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center z-30" style={{ background: `color-mix(in srgb, var(--clr-bg) 94%, transparent)`, backdropFilter: 'blur(4px)' }}>
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={handleShare}
          className={`font-black py-3 px-5 rounded-xl flex items-center gap-2 pointer-events-auto ${neoBtn}`}
        ...
        <button
          onClick={onMenu}
          className={`font-bold py-3 px-5 rounded-xl flex items-center gap-2 pointer-events-auto ${neoBtn}`}
